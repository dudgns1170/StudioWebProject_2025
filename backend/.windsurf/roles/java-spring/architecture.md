...
trigger:
  ...
  project_detection:
    - "build.gradle"
    - "application.yml"
description: Spring Boot 3-Layered 아키텍처 가이드
globs: ["**/*.java", "**/*.yml", "**/*.yaml"]
conditional_load:
  if_exists:
    - "src/main/java"
...
- **[문서와 코드의 일관성]**
  - [BACKEND_API_AND_STRUCTURE.md](BACKEND_API_AND_STRUCTURE.md)의 엔드포인트/요청/응답 예시를 기준으로  
...
- **[환경 변수]**
  - [.env](.env) 파일에 DB, JWT, Redis, AWS S3, Mail 등 민감 정보를 정의하고  
    `application.yml`에서 `${...}` 형태로 참조
- **[보안]**
  - JWT, DB 패스워드, AWS 키 등은 git에 커밋하지 않고,  
    [.env](.env) / 환경 변수 / 서버 설정으로만 관리한다.
